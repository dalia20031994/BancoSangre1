//para ver el mapa con los donadores existentes segun los filtros que se apliquen 
import React, { useEffect, useState, useContext } from 'react';
import L from 'leaflet';
import 'leaflet/dist/leaflet.css';
import { AuthContext } from '../../auth/AuthContext';
import { fetchDonadores } from '../../api/donador.api';
import DonadorMapa from '../../components/Donador/FormComponents/IconoDonadorMapa';
import FiltroDonadores from '../../components/Donador/FormComponents/DonadorFiltro';
const MapaDonadores = () => {
  ...
  const loadDonadores = async () => {
    ...
    setError(null);
    try {
      const data = await fetchDonadores(token, filters);
      setDonadores(data);
    } catch (err) {
      setError(err);
    } finally {
          ...
          <div class="w-[260px] font-sans text-[#2c3e50] p-4">
            ...
            <div class="mt-3 text-sm leading-relaxed bg-gray-50 rounded-lg p-3 border border-gray-200">
              ...
              <p class="my-1"><strong>📧 Correo:</strong> 
                <a href="mailto:${d.correo}" class="text-blue-700 hover:underline">${d.correo}</a>
              </p>
            </div>
          </div>
        `);
