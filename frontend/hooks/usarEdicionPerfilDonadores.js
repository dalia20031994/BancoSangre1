...
const usarEdicionPerfilDonadores = (setMensaje) => {
  ...
  const { id } = useParams(); // ID del donador desde la URL
  ...
  const cargarDatosIniciales = useCallback(async () => {
    try {
      // 1. Obtener datos del donador específico
      ...
      // 3. Obtener datos de dirección si existe
      ...
      setEsValidoRol(true); // Asumimos permisos si se cargaron los datos
    } catch (error) {
